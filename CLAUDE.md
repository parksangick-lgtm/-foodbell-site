...
볼트 `%USERPROFILE%\OneDrive\chefpark\푸드벨-업무\` 에 행사 기록과 진행 중인 일을 적는다(2026-09-25 시작).

- **"행사 준비 기록해 줘"** → `_틀/행사.md` 를 복사해 `행사/YYYY-MM-DD 행사 (지역).md` 를 만들고 "행사 전" 을 채운다.
- **"행사 후기 기록해 줘"** → 같은 노트의 "행사 후" 를 채운다.
- **"진행 상황 정리해 줘" / "뭐 남았지?"** → `진행중.md` 를 고치거나 읽어 준다. 끝난 일은 "끝난 일" 로 내린다.
- 한 행사에 **시제상과 출장음식을 함께 주문**하는 일이 많아 양식은 하나다. 제사상은 상차림(몇 상·몇 열 진설·음복), 출장음식은 참석 인원으로 적고, 주문에 없는 칸은 지운다.
- 노트 위쪽 "주문자" 에 **이름·연락처를 적는다**(사용자 결정, 2026-09-25). 개인 볼트 안에서만 쓰고,
  이 저장소·사이트·견적서 예시로는 절대 옮기지 않는다. 파일 제목에는 넣지 않는다(지역만).
- 노트 끝에 `[[시제]]` `[[지역]]` 링크를 달아 옵시디언 그래프 보기로 잇는다. 행사 노트가 30장쯤 쌓이면 Graphify 를 시험해 본다.
