...
**Snippet drift inside one bundle — a grep, not a re-reading.** A snippet
duplicated across a bundle's own files is the same shared-rule problem one
level down: a fix applied to one copy leaves the others live with the bug,
and the copy an agent adapts from is whichever file it was told to load,
not the one that got fixed. Keep one canonical copy (SKILL.md's), point
the others at it, and check for the retired forms mechanically. Run over
the staged bundle; any hit fails the audit:

```bash
# The trailing double-quote in the pattern is load-bearing: a real snippet
# passes find a quoted path, while the prose that NAMES the retired form
# writes it with an ellipsis instead. Matching on the quote separates the
# two; without it, this check fails forever on the rule's own statement of
# itself — and on any comment that quotes an example of it.
grep -rn 'for f in \$(find "' "$s" && echo "DRIFT: retired word-splitting loop still present"
# both files must carry the sweep guard; -L lists the ones that do NOT
grep -rL 'ARCHIVAL SWEEP BROKEN' "$s/SKILL.md" "$s/references/observation-log.md"
```

Add the retired form to this list in the SAME edit that corrects a
snippet — a drift check written after the second divergence is discovered
has already let the first one ship. And run the new pattern against both
controls before trusting it: a **positive** control (the retired snippet
as it actually appears in the bundle) and a **negative** one (the prose
that names the retired form). A drift check derived from how the author
remembers the bad form, rather than from the bundle's literal bytes, is
how a check comes to fail permanently on documentation — which trains
whoever runs it to ignore the finding. (Observed: the first version of
this very grep fired on SKILL.md's own sentence describing the rule.)
...
# Append the manifest entry NOW, in this same block — not at Step 8 (see below).
# A seed with no entry is indistinguishable from an installed copy.
# then make EVERY edit against the staged path
```

**Write the manifest entry at SEED time, not at delivery.** The Delivery
section's ordering — stage, edit, then append to `PENDING.md` at Step 8 —
guarantees that a session which stops between the seed and the edit leaves a
staged copy no manifest mentions. That copy is not merely undocumented, it is
**unclassifiable**: the same-day rule's three outcomes all turn on whether an
entry exists, and `diff -rq` cannot separate "seeded, never edited" from
"edited and already installed" because **both are byte-identical to live**. The
state that distinguishes them exists nowhere on disk. (Observed: a review found
`skill-updates/<date>/<skill>/` already present with no `PENDING.md` at all, and
had to fall back to comparing mtimes — live older than the copy — to conclude
it was an abandoned seed.)

So the seed and its entry are ONE act, for the same reason archival rides
inside the id-derivation command rather than sitting beside it in prose: a duty
attached as a preamble (or a postscript) to another action inherits none of that
action's enforcement. At seed time the entry carries what is already known — the
skill, the anchor directory, the producer, and `status: seeded`; Step 8 then
updates that entry in place with the observation ids and the per-change summary
rather than creating it.

Add a fourth outcome to the same-day rule: **manifest absent, or no entry for
this skill, and the copy is identical to live → an abandoned seed; it is safe to
continue editing in place.** Without it the rule has no branch that matches, and
the reviewer is left inferring intent from timestamps.

The general form, which reaches past this skill: **the artefact you hand over
and the list that says you handed it over must be produced by the same act.**
Defer the list to the end of the job and every interrupted job leaves an object
that is not "missing" to the next person — it is *unidentifiable*. And where a
state cannot be recovered from the artefact's own content, the record of that
state is not documentation, it is the only copy.
