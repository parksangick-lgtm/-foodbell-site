...
> **SKILL.md holds the canonical copy of this snippet and of the id
> derivation below.** They are duplicated here because this is the file an
> agent loads when setting up the log or changing how anything reads it —
> which is exactly why a fix applied to one copy and not the other leaves
> the adaptation starting from the unsafe version. If they ever disagree,
> SKILL.md wins; sync this file rather than improving it in place.
