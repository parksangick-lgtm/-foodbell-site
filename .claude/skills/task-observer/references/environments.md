...
## Before installing — search every scope, end with exactly one copy

Search-before-create applies to installed artefacts as much as to data: a
second copy of a skill at another scope is a silent fork of the live file.
This section already requires it for the log ("Before creating a log,
search the plausible anchors for an existing one and adopt it"); the
bundle needs the same rule, and for the same reason.

1. **Before copying anything**, list every skills-discovery scope the
   harness reads — user (`~/.claude/skills/`), project
   (`<project>/.claude/skills/`), plugin caches, synced or managed skill
   directories — and look for a directory with this skill's name in each:

   ```bash
   find ~/.claude "<project>/.claude" -maxdepth 4 -type d -name 'task-observer' 2>/dev/null
   ```

2. **If one exists, decide deliberately**: update that copy in place, or
   remove it and install at the other scope. Never do both and leave both.
3. **After installing, confirm exactly one copy resolves.** Re-run the
   search; more than one hit is an incomplete install, not a success.

The README's project-level and user-level instructions are a **choice, not
a sequence** — running one after the other is what produces the second
copy. (Observed: a full bundle was installed at user scope, then the
README's project-level commands were run as if they were the next step,
creating a second copy carrying `SKILL.md` and `references/` but no
`scripts/`. Two same-name copies make "the live file" ambiguous for this
skill's own staging workflow — the review's `diff -rq` staged-vs-live
reconciliation has no single answer — and the partial copy loads as an
incomplete bundle in that project. Nothing detected it; it surfaced only
because an agent happened to diff the two.)
...
# The staleness cutoff. Without it this hook implements only HALF the rule it
# enforces (Session Start step 3: `never` OR older than 7 days) — and the half
# it drops is the one that matters from the second review onwards.
cutoff=$(date -d '7 days ago' +%F 2>/dev/null || date -v-7d +%F 2>/dev/null)
...
**Run the template against those three fixtures before shipping it, and
state what each must produce:** `never` → offer, 30 days stale → offer,
2 days stale → silent. The `never`-only version of this template passed a
fresh install's test and then went silent permanently the moment the first
review wrote a date — the self-concealing failure this very section warns
about, shipped inside the enforcement meant to prevent it. An enforcement
template shipped beside a rule must implement the rule's whole trigger
condition, and the fixtures that prove it are the ones the surrounding
prose already prescribes.
