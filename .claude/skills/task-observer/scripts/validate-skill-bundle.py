...
def _force_utf8_output():
    """Never assume the console is UTF-8.

    On a Windows console with a CJK code page (cp949/cp932/cp936), printing a
    line containing an em dash raises UnicodeEncodeError and aborts the run
    part-way through. That abort exited 1 — the status reserved for "a gate
    check failed" — so a broken instrument was indistinguishable from a bad
    bundle. Reconfigure where the runtime allows it; fall back silently.
    """
...
if __name__ == "__main__":
    _force_utf8_output()
    # Exit codes are four-valued on purpose, so a crash can never masquerade
    # as a verdict about the thing being checked:
    #   0 = all gate checks passed
    #   1 = at least one gate check failed
    #   2 = usage error (no skill directory given)
    #   3 = the validator itself broke
    try:
        sys.exit(main(sys.argv))
    except SystemExit:
        raise
    except BaseException as exc:
        print(f"VALIDATOR ERROR: {type(exc).__name__}: {exc}")
        print("  (the validator crashed - this is NOT a verdict about the bundle)")
        sys.exit(3)
